import 'package:flutter/material.dart';

class AppColor {
  static const Color whiteColor = Color(0xFFFFFFFF);
  static const Color blackColor = Color(0xFF000000);
  static const Color blackLightColor = Color(0xFF053F1D);
  static const Color primaryButtonColor = Color(0xff0A66C2);
  static const Color secondaryButtonColor = Color(0xff2470B8);
  static const Color primaryColor = Color(0xff0A66C2);
  static const Color primaryLightColor = Color(0xFFB3D0EC);
  static const Color borderSideColor = Color(0xffCCCCCC);
  static const Color greenColor = Color(0xFF27AE60);
  static const Color orangeColor = Color(0xFFE67E22);
  static const Color redColor = Color(0xFFE74C3C);
  static const Color blueColor = Color(0xFF2980B9);
  static const Color pinkColor = Color(0xFF9B59B6);



  static const Color tabBarBackgroundColor = Color(0xffB3C5D0);
  static const Color cardBackgroundColor = Color(0xffF6F8FA);
  static const Color bodyTextColor = Color(0xff313E40);
  static const Color hintColor = Color(0xff5C6F73);
  
  static const Color greyColor = Colors.grey;
  static const Color darkBlueColor = Color(0xff111B2B);
}
